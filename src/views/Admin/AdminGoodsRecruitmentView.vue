            ...
            <nav class="flex mb-5" aria-label="Breadcrumb">
              <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                ...
                <li>
                  <div class="flex items-center">
                    <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                    <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500" aria-current="page">招领管理</span>
                  </div>
                </li>
              </ol>
            </nav>
            <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">招领管理</h1>
          </div>
...
<!--            <a href="#" id="createProductButton" class="text-white bg-blue-500 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-primary-600 dark:hover:bg-blue-500 focus:outline-none dark:focus:ring-primary-800" type="button" >-->
<!--              新增信息-->
<!--            </a>-->
          </div>
        </div>
...
<!--                <tr v-for="card in paginatedCards" :key="card" class="hover:bg-gray-100 dark:hover:bg-gray-700">-->
                <tr v-for="(card, index) in paginatedCards" :key="index" class="hover:bg-gray-100 dark:hover:bg-gray-700">
                  <td class="w-4 p-4">
                    <!--                                <div class="flex items-center">-->
                  ...
                  <td class="max-w-sm p-4 overflow-hidden text-base font-normal text-gray-500 truncate xl:max-w-xs dark:text-gray-400">{{ card.realName }}</td>
                  <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ card.phoneNumber }}</td>
                  <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ card.roleNumber }}</td>
                  <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ card.claimTime }}</td>
                  <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                    <div class="mt-2.5 mb-5">
                      <span v-if="card.stuffState" class="bg-green-300 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded dark:bg-green-300 dark:text-blue-800 ms-3">已完成</span>
                      <span v-else class="bg-blue-300 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded dark:bg-red-300 dark:text-blue-800 ms-3">已完成</span>
                    </div>
                  </td>
...
const fetchData = async () => {
  try {
    axios.get("/goods/listCla")
        .then(response => {
          // 假设后端返回的数据是 response.data.data.list
          console.log("xxxxxxxx",response.data.data)
          cards.value = response.data.data.map((item: BackendData) => ({
            goods: item.goods,

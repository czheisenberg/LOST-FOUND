...
<!--    </div>-->
    <div class="flex flex-col md:flex-row justify-stretch items-start p-4 md:p-34">
      <div class="bg-gray-100 dark:bg-gray-700 h-full rounded-lg mb-4 md:mb-0 mx-auto md:mx-0">
        <img :src="fileUrl" class="max-w-full max-h-full object-contain scale-75"/>
      </div>
      <div class="w-full md:w-1/2 lg:w-1/3 xl:w-1/4 px-4 mb-4">
        <div v-for="(card, index) in goodsRes" :key="index" class="w-full h-full px-4 mb-4">
          <div class="bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            ...
            <a>
              <img class="bg-gray-100 dark:bg-gray-700 object-contain w-full h-64 rounded-t-lg" :src="card.goodsimg" alt="Product Image"/>
            </a>
            <div class="p-4">
              <a>
                <h5 class="text-xl font-semibold tracking-tight text-gray-900 dark:text-white">{{ card.message }}</h5>
              ...
              <div class="flex items-center justify-between">
                <span class="text-xs font-bold text-gray-900 dark:text-white">联系电话: {{ card.phonenumber }}</span>
                <router-link class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" :to="{ name: 'details', params: { id: card.goodsId, message: card.message } }">详细信息
                </router-link>
              </div>
            </div>
          </div>

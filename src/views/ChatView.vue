...
async function registerUser(userID: any, username: any) {
  ...
  // 判断账户是否注册
  let response = await axios.post('https://console.tim.qq.com/v4/im_open_login_svc/account_check?sdkappid=' + 1600044444 + '&identifier=administrator&usersig=' + userSig + '&random=2949672&contenttype=json'
      , {
        "CheckItem": [
          {
            "UserID": userID
...
onBeforeMount(async () => {
  ...
  try {
    ...
    // 注册访问者
    response = await registerUser(userID, username);
